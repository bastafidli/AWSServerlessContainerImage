...
WORKDIR /src
COPY ["AWSServerlessContainerImage.csproj", "AWSServerlessContainerImage/"]
RUN dotnet restore "AWSServerlessContainerImage/AWSServerlessContainerImage.csproj"

WORKDIR "/src/AWSServerlessContainerImage"
COPY . .
RUN dotnet build "AWSServerlessContainerImage.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "AWSServerlessContainerImage.csproj" \
...
Execute unit tests
```
    cd "AWSServerlessContainerImage/test/AWSServerlessContainerImage.Tests"
    dotnet test
```

Deploy application
```
    cd "AWSServerlessContainerImage/src/AWSServerlessContainerImage"
